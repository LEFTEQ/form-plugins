...
html_to_tokens = function (html) {
  let char, current_word, i, len, mode, words;
  mode = 'char';
  current_word = '';
  words = [];
...
find_matching_blocks = function (before_tokens, after_tokens) {
  let index_of_before_locations_in_after_tokens, matching_blocks;
  matching_blocks = [];
  ...
  for (index = i = 0, len = matches.length; i < len; index = ++i) {
    ...
    if (action_up_to_match_positions !== 'none') {
      operations.push({
        ...
        end_in_before:
          action_up_to_match_positions !== 'insert' ? match.start_in_before - 1 : void 0,
...
wrap = function (tag, content) {
  let length, non_tags, position, rendering, tags;
  rendering = '';
  position = 0;
...
op_map.replace = function (op, before_tokens, after_tokens) {
  return (
    op_map.insert(op, before_tokens, after_tokens) + op_map.delete(op, before_tokens, after_tokens)
  );
};
